    ...
    <div class="flex items-center gap-4">
      <div class="relative hidden sm:block">
        ...
        <input v-model="searchQuery" type="text" placeholder="搜尋電影、演員..."
          ...
          @focus="isFocused = true" @blur="handleBlur" @keyup.enter="handleEnter" />
        <!-- 下拉搜尋結果 -->
        <div v-if="showDropdown"
          ...
          <template v-else-if="searchResults?.results?.length">
            <RouterLink v-for="movie in searchResults.results.slice(0, 6)" :key="movie.id"
              :to="{ name: 'movie-detail', params: { id: movie.id } }"
              class="flex items-center gap-3 px-4 py-3 hover:bg-slate-800 transition-colors"
              @click="() => { searchQuery = ''; isFocused = false }">
              <img v-if="movie.poster_path" :src="`${IMAGE_BASE_URL}/w92${movie.poster_path}`" :alt="movie.title"
